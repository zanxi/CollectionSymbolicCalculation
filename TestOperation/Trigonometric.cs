...
namespace TestOperation
{
    public class Sin : Function
    {
        ...
        static MathObject SinProc(params MathObject[] ls)
        {
            ...
            if (u is Number && u < 0) return -Constructors.sin(-u);

            if (u is Product &&
                (u as Product).elts[0] is Number &&
                (u as Product).elts[0] < 0)
                return -Constructors.sin(-u);
                ...
                (u as Product).elts[1] == Pi)
            {
                ...
                if (n > 2) return Constructors.sin(Mod(n, 2) * Pi);

                if (n > 1) return -Constructors.sin(n * Pi - Pi);

                if (n > half) return Constructors.sin((1 - n) * Pi);
            ...
            if (u is Sum && (u as Sum).elts.Any(Product_n_Pi))
            {
                ...
                return Constructors.sin((u - pi_elt) + Mod(n, 2) * Pi);
            }
    ...
    public class Cos : Function
    {
        ...
        static MathObject CosProc(params MathObject[] ls)
        {
                ...
                )
            {
                ...
                if (n > 2) return Constructors.cos(Mod(n, 2) * Pi);

                if (n > 1) return -Constructors.cos(n * Pi - Pi);

                if (n > half) return -Constructors.cos(Pi - n * Pi);
            ...
            if (ls[0] is Sum && (ls[0] as Sum).elts.Any(Product_n_Pi))
            {
                ...
                return Constructors.cos((ls[0] - pi_elt) + Mod(n, 2) * Pi);
            }
